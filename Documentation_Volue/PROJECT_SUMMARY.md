...
- ❌ **Redis** (no distributed caching)
- ❌ **Apache Pulsar** (no messaging/event streaming)
- ❌ **Electric.Core Pulsar features** (only uses CDC tracking)

**This document describes SHARED INFRASTRUCTURE.** Other services (NotificationService) may use Redis/Pulsar. Architecture diagrams show infrastructure capabilities, not ProductionForecast-specific implementation.

---

## Executive Summary

SmartPulse is an enterprise-grade platform for managing electricity market data. ProductionForecast service uses simple local IMemoryCache with CDC-based invalidation (NOT Redis distributed cache, NOT Pulsar messaging). NotificationService and infrastructure may use advanced distributed features. Built with .NET 7/8/9.
...
- **ProductionForecast Caching**: IMemoryCache (local in-memory cache ONLY) + OutputCache middleware
- **ProductionForecast CDC**: Electric.Core for Change Data Capture (cache invalidation trigger)
- **ProductionForecast Messaging**: NONE (no Pulsar, no message bus)
- **Note**: Other services (NotificationService) may use Redis/Pulsar - this summary focuses on infrastructure capabilities, NOT ProductionForecast specifics
...
**⚠️ CRITICAL NOTE**: The diagrams below show **SHARED INFRASTRUCTURE CAPABILITIES**. **ProductionForecast service uses ONLY CDC Trackers + IMemoryCache** (no Pulsar, no Redis). NotificationService uses full infrastructure.
...
#### **ProductionForecast Architecture (ACTUAL Implementation)**

```mermaid
%%{init: {'theme':'base', 'themeVariables': { 'fontSize':'16px', 'primaryColor':'#27ae60'}}}%%
graph TB
    subgraph Clients["🖥️ CLIENT LAYER"]
        A["Web, Mobile, Desktop"]
    end

    subgraph API["📊 ProductionForecast API"]
        B["ASP.NET Core v2<br/>+ OutputCache Middleware"]
    end

    subgraph Cache["💾 LOCAL CACHE"]
        IMEM["IMemoryCache<br/>(In-Memory)"]
        OUT["OutputCache<br/>(HTTP Response)"]
    end

    subgraph Infrastructure["⚙️ ELECTRIC.CORE"]
        H["CDC Trackers<br/>(1s polling)"]
    end

    subgraph External["🚀 EXTERNAL"]
        L["SQL Server<br/>(Database + CDC)"]
    end
    ...
    style B fill:#3498db,stroke:#2c3e50,color:#fff,stroke-width:3px
    style IMEM fill:#27ae60,stroke:#2c3e50,color:#fff,stroke-width:3px
    style OUT fill:#27ae60,stroke:#2c3e50,color:#fff,stroke-width:3px
    style H fill:#e67e22,stroke:#2c3e50,color:#fff,stroke-width:3px
    style L fill:#e74c3c,stroke:#2c3e50,color:#fff,stroke-width:3px
```

**ProductionForecast Components**:
- ✅ **IMemoryCache**: Local in-memory cache (per pod)
- ✅ **OutputCache**: HTTP response caching middleware
- ✅ **Electric.Core CDC**: Database change polling (1-second interval)
- ❌ **NO Pulsar**: No message bus
- ❌ **NO Redis**: No distributed cache

---

#### **Complete Infrastructure View (All Services)**

**Note**: This diagram shows FULL infrastructure. Only NotificationService uses Pulsar/Redis.
...
graph TB
    ...
    subgraph APIs["📊 API LAYER"]
        B["ProductionForecast<br/>(IMemoryCache ONLY)"]
        C["NotificationService<br/>(Redis + Pulsar)"]
        D["GraphQL"]
    end
...
- **ProductionForecast Communication**: REST API only (no async messaging, no Pulsar)
- **ProductionForecast Consistency**: CDC-based cache invalidation (no Redis)

### 2. **Caching (ProductionForecast)**
- **IMemoryCache**: Local in-memory cache (cache-aside pattern)
- **OutputCache**: ASP.NET Core middleware for HTTP response caching
- **NO Redis**: ProductionForecast does NOT use distributed cache
- **NO L2/L3**: Simple local caching only

### 3. **Change Data Capture (CDC) - ProductionForecast**
- **Mechanism**: SQL Server Change Tracking + Electric.Core polling
- **Trigger**: Database writes → CDC table → ChangeTracker polls → Invalidate IMemoryCache
- **Effect**: Automatic local cache invalidation (no Redis, no Pulsar events)
- **ProductionForecast**: Uses Electric.Core ONLY for this CDC functionality
