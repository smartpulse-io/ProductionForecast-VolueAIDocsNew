...
### 🔧 Components
- **[Electric.Core Framework](./docs/components/electric_core.md)** - Infrastructure library for CDC (Change Data Capture) - NOTE: ProductionForecast uses ONLY CDC functionality, NOT Pulsar/messaging features
...
- **[Business Logic & Caching](./docs/components/production_forecast/business_logic_caching.md)** - IMemoryCache (local in-memory cache), cache invalidation via CDC, OutputCache middleware
...
### 🔌 Integration
<!-- NOTE: ProductionForecast does NOT use Pulsar or Redis - these are infrastructure capabilities used by OTHER services -->
- **[Pulsar Integration](./docs/integration/pulsar.md)** - Message bus (NOT used by ProductionForecast)
- **[Redis Integration](./docs/integration/redis.md)** - Distributed cache (NOT used by ProductionForecast)
...
### By Technology
- **Apache Pulsar** → NOT used by ProductionForecast (infrastructure capability for other services)
- **Redis** → NOT used by ProductionForecast (infrastructure capability for other services)
- **IMemoryCache** → ProductionForecast uses local in-memory cache ONLY
...
- **Reliability** → `docs/guides/troubleshooting.md`
- **Security** → `docs/guides/code_review.md` (security checks), `docs/integration/api.md`
- **Caching (ProductionForecast)** → IMemoryCache + OutputCache only (see `docs/components/production_forecast.md`)
- **Real-time Updates (ProductionForecast)** → CDC-based cache invalidation (see `docs/data/cdc.md`)

### By Service/Component
- **ProductionForecast Service**
  - Architecture: `docs/components/production_forecast.md`
  - API: `docs/integration/api.md` (endpoints)
  - Caching: IMemoryCache + OutputCache ONLY (see `docs/components/production_forecast.md`)
  - Database: `docs/data/ef_core.md` (DbContext)
  - CDC: `docs/data/cdc.md` (cache invalidation via Electric.Core CDC tracking)
...
- **Electric.Core Library**
  ...
  - CDC (Change Data Capture): `docs/data/cdc.md` - **ProductionForecast uses ONLY this feature**
  - Pulsar: `docs/integration/pulsar.md` - NOT used by ProductionForecast
  - Collections: `docs/components/electric_core.md`, `docs/patterns/concurrency.md` - NOT used by ProductionForecast
  - Distributed Data: `docs/patterns/distributed_data.md` - NOT used by ProductionForecast
...
2. **Add CDC tracker**: `docs/data/cdc.md` (cache invalidation trigger)
3. **Test invalidation**: Verify CDC triggers cache expiration
4. **Note**: ProductionForecast does NOT use distributed cache (Redis) or messaging (Pulsar)

#### Scale the system
1. **Read**: `docs/architecture/00_system_overview.md` (scalability strategies)
2. **Horizontal (ProductionForecast)**: Each instance has own IMemoryCache, CDC invalidates all instances
3. **Vertical**: Review concurrency patterns
4. **Database**: `docs/data/ef_core.md` (bulk operations, indexes)
5. **Note**: Other services (NotificationService) may use Redis/Pulsar for scaling
...
- **Cache not invalidating (ProductionForecast)**: See `docs/data/cdc.md` (CDC-based invalidation only)
