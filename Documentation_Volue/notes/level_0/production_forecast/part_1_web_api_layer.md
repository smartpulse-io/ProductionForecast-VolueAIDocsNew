...
- ✅ Electric.Core for CDC (Change Data Capture - cache invalidation)
...
// Services registration (ProductionForecast)
builder.Services.AddSmartPulseApplicationCommonServices(configuration);
// NOTE: ProductionForecast does NOT register:
// - AddApachePulsarClient (no Pulsar)
// - AddDistributedDataManager (no Redis)
// Only registers: IMemoryCache, CDC trackers, EF Core
